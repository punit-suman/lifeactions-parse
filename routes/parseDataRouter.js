...
const parseData = async() => {
    try {
        ...
        if (sqlConn) {
            ...
            finalDataTbl.columns.add('eventInfo', sql.VarChar(sql.MAX), {nullable: true})
            finalDataTbl.columns.add('package_name', sql.VarChar(sql.MAX), {nullable: true})
            finalDataTbl.columns.add('data_text', sql.VarChar(sql.MAX), {nullable: true})
            finalDataTbl.columns.add('data_description', sql.VarChar(sql.MAX), {nullable: true})
            ...
            await insFinalDataReq.bulk(finalDataTbl)
        }
    } catch(err) {
